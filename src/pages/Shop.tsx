...
// We'll use a simplified type for the Product in the Shop component
type Product = {
  id: number;
  name: string;
  price: number;
...
export default function Shop() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedFilters.size?.length) {
      filtered = filtered.filter(product => 
        selectedFilters.size.some(size => product.sizes.includes(size as ProductSize))
      );
    }
    
    if (selectedFilters.color?.length) {
      filtered = filtered.filter(product => 
        selectedFilters.color.some(color => product.colors.includes(color as ProductColor))
      );
    }
  ...
  const FilterSection = ({ title, items, category }: { title: string; items: FilterItem[]; category: string }) => (
        ...
        {items.map((item) => {
          ...
          return (
            <label key={value} className="flex items-center space-x-3 cursor-pointer group">
              ...
              <span className="text-sm text-paper group-hover:text-accent transition-colors">
                {label}
              </span>
            </label>
          );
    ...
    >
      <CardContent className="p-0 relative">
        ...
        <div className="absolute top-3 left-3 z-10 flex flex-col gap-2">
          {product.isNew && (
            <Badge className="bg-accent text-ink font-medium text-xs px-2 py-1">NEW</Badge>
          )}
          {product.compareAt && (
            <Badge variant="destructive" className="bg-red-600 text-white font-medium text-xs px-2 py-1">
              SALE
            </Badge>
          )}
        </div>
        ...
        <div className="aspect-[4/5] overflow-hidden bg-mink/10 relative">
          ...
          {!product.inStock && (
            <div className="absolute inset-0 bg-graphite/50 flex items-center justify-center">
              <Badge variant="secondary" className="bg-bg text-graphite font-medium">
                Out of Stock
              </Badge>
            </div>
          )}
        ...
        <div className="p-4 space-y-3">
          <div className="space-y-1">
            <h3 className="font-medium text-paper group-hover:text-accent transition-colors leading-tight">
              {product.name}
            </h3>
          ...
          <div className="flex items-center space-x-2">
            ...
            {product.compareAt && (
              <span className="text-sm text-graphite line-through">${product.compareAt.toLocaleString()}</span>
            )}
            {product.compareAt && (
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-1">
              {product.colors.slice(0, 4).map((color: string) => (
                <div
                  key={color}
                  className="w-4 h-4 rounded-full border border-graphite/30 hover:scale-110 transition-transform cursor-pointer"
                  style={{ 
              ...
              {product.colors.length > 4 && (
                <span className="text-xs text-graphite ml-1">+{product.colors.length - 4}</span>
              )}
            </div>
      ...
      <main className="container mx-auto px-4 pt-24 pb-16">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-12">
          <div className="space-y-2">
            <h1 className="text-5xl font-display font-medium text-paper mb-2 tracking-tight">Shop</h1>
            <p className="text-lg text-graphite max-w-md">Discover our curated collection of contemporary essentials crafted for the modern individual</p>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
              ...
              <SheetTrigger asChild>
                <Button variant="outline" className="w-full justify-center border-graphite/30 hover:border-accent/50">
                  ...
                  {Object.keys(selectedFilters).length > 0 && (
                    <Badge className="ml-2 bg-accent text-ink px-2 py-0.5 text-xs">
          ...
          <aside className="hidden lg:block w-72 space-y-8">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={clearFilters} variant="ghost" size="sm" className="text-graphite hover:text-accent">
                Clear All
              </Button>
            </div>
                  ...
                  {Object.entries(selectedFilters).map(([category, values]) =>
                    values.map((value) => (
                      <Badge
                        key={`${category}-${value}`}
                        variant="secondary"
                        className="mr-2 mb-1 cursor-pointer hover:bg-accent hover:text-ink transition-colors"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredProducts.length > 0 ? (
                ...
                <div className="col-span-full py-12 text-center">
                  <div className="text-graphite text-xl mb-4">No products found</div>
                  <p className="text-sm text-graphite/60 max-w-md mx-auto">
                    Try adjusting your search or filter criteria to find what you're looking for.
                  </p>
                  <Button 
                    variant="outline" 
                    className="mt-6 border-graphite/30 hover:border-accent/50"
